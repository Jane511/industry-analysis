...
def main() -> None:
    """Run the canonical contract-export pipeline on real public data.

    The engine uses only real public sources (ABS / RBA / PTRS); there is no
    synthetic-data path. If a required input is not staged, the relevant
    builder surfaces it rather than silently exporting zeros.
    """
    try:
