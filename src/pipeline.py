...
def run_pipeline() -> None:
    ...
    foundation = build_foundation(RAW_PUBLIC_DIR_ABS, PROCESSED_INDUSTRY_DIR)
    macro = build_macro_view(foundation, RAW_PUBLIC_DIR_ABS, PROCESSED_INDUSTRY_DIR)
    benchmarks = build_industry_benchmarks(macro, PROCESSED_INDUSTRY_DIR)
    save_csv(benchmarks, OUTPUT_TABLES_DIR / 'industry_generated_benchmarks.csv')
    borrower_compare = build_bottom_up(macro, benchmarks, PROCESSED_INDUSTRY_DIR)
    ...
    scorecard = build_scorecard(borrower_compare, PROCESSED_INDUSTRY_DIR, OUTPUT_TABLES_DIR)
    ...
    appetite = build_industry_credit_appetite_strategy(macro, PROCESSED_INDUSTRY_DIR)
    save_csv(appetite, OUTPUT_TABLES_DIR / 'industry_credit_appetite_strategy.csv')

    stress = build_industry_stress_test_matrix(macro, PROCESSED_INDUSTRY_DIR)
    save_csv(stress, OUTPUT_TABLES_DIR / 'industry_stress_test_matrix.csv')

    esg_overlay = build_industry_esg_overlay(macro, PROCESSED_INDUSTRY_DIR)
    ...
    portfolio = build_portfolio_proxy(macro, PROCESSED_INDUSTRY_DIR)
    ...
    workbook_path = build_reporting_workbook(
        ...
        watchlist,
        stress,
        PROCESSED_INDUSTRY_DIR / 'industry_risk_reporting_workbook.xlsx',
    ...
    # --- Stage 7: property and industry reference-layer outputs ---
