...
def _estimate_inventory_days(row: pd.Series) -> float:
    ...
    if pd.notna(inventory_ratio):
        ...
        return round(float(inventory_ratio) * 365 / cogs_ratio, 1)
...
def build_industry_benchmarks(public_df: pd.DataFrame, processed_dir: Path) -> pd.DataFrame:
    df = public_df.copy()
    ...
    df["ar_days_benchmark"] = _clip_round(ar_base, 5, 75)
    ...
    df["ap_days_benchmark"] = _clip_round(ap_base, 20, 70)
    ...
    benchmark["benchmark_origin"] = (
        ...
        "other benchmarks estimated from public industry signals using deterministic bank-style rules"
    )
