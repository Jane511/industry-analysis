...
Headline picture: the cash rate is 4.35% (+0.50 points over the past year) and the arrears backdrop is low and improving, so the macro regime is set to 'base'. Against that backdrop, 5 of 18 industries score Elevated on the 1-5 risk scale, and 0 of 5 property segments are in downturn. The downturn-overlay table translates this into stress multipliers — up to 2.00x on PD in a severe scenario — that a pricing or expected-loss model can apply directly.

Macro and downturn overlays are dated 2026-06-15; property-cycle data is dated 2026-03-01. The headline numbers are in Section 2; every figure traces back to a named public source in Sections 3-5.
